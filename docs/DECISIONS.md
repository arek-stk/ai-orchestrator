...
* **Addendum (2026-09-14) — milestones drive releases:**
  * Roadmap milestones are named `vX.Y — <theme>`; `Backlog — proposed` is never released. When the lowest open
    `vX.Y` milestone has no open and at least one closed item, `milestone-release.yml` targets `X.Y.0`: if the
    release-please PR has another version it first opens a bot PR with an empty `Release-As: X.Y.0` commit. After the
    release is published it appends the milestone link to the notes, comments on the release PR and closes the
    milestone.
  * **Safe mode (default):** the automation prepares everything up to a release PR that is ready to merge (Release-As
    targeting, CI on bot PRs, a "ready to merge" comment, release notes, milestone close) but never enables auto-merge
    and never merges into `main`; a maintainer merges the Release-As and release PRs.
  * **Full mode (opt-in):** only when the repository variable `AUTO_RELEASE_MERGE` is exactly `true`, the workflow
    enables squash auto-merge on the Release-As and release PRs. Trade-off: `main` protection has no required reviews
    and does not enforce admins, so in full mode the bot lands release commits without any human. Safe mode is
    recommended; enable full mode only after adding required reviews or moving the automation to a dedicated GitHub
    App whose merges are reviewed.
  * Bot PRs get CI without a personal token: their `pull_request` runs wait for approval, so the workflow dispatches
    `ci.yml` (`workflow_dispatch` is exempt from the GITHUB_TOKEN trigger rule) once per head commit, re-reading the
    branch tip right before dispatching and counting only check runs for that head SHA. It dispatches release-please
    when a bot merge left the default branch head unprocessed. A PAT or GitHub App token was rejected as a standing
  ...
  * Safety: decisions live in the pure, unit-tested `.github/scripts/milestone-release/logic.mjs`. Issue and PR events
    start the write-capable job only when a milestone is involved and the actor has write, maintain or admin
    permission (checked by a read-only job); schedule, manual and milestone events are trusted. Only bot-authored PRs
    from this repository are ever auto-merged; auto-merge enabled by a human is never withdrawn; a merged or closed
    Release-As PR is never recreated. Pause everything with `AUTO_RELEASE=false` or the `release:hold` label on the
    release PR. PRs get a milestone only from a `milestone:vX.Y` label or a closing reference to an issue in that
    milestone.
