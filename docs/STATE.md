...
| — | Supply-chain hardening (Scorecard): actions pinned by commit SHA, server base image pinned by digest, Copilot CLI pinned to an exact version, least-privilege job tokens (ci/release workflows follow in PR #19) | ✅ done | Scorecard still flags `npm install` without a lockfile (Copilot CLI, server `dist`) |
...
| 22 | Autopilot stage 1 (ADR-034): time- and budget-boxed sessions, capped autonomy (never persisted), hard gates, `PARKED` runs without a slot, deferred approval expiry, stop conditions + kill switch, deterministic "While you were away" digest, migration `0003` (`autopilot_sessions`, session projects, run/approval columns), `/api/autopilot/*`, start dialog, banner, digest page | 🚧 in review | 39 core tests (pipeline scenarios incl. `maxConcurrentTasks = 2` and a parked dependency addition) · 4 PGlite tests · 7 API tests · web typecheck + build |
...
3. 🚧 **Autopilot / away mode** (stage 1 built, ADR-034; stages 2–5 open): time- and budget-boxed unattended work. Questions are resolved via decision memory, then research, then an agent council with a critic on a different model; anything risky is parked for the human; there is a return digest. See `docs/plans/autopilot.md`.
