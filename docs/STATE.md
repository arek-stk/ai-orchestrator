...
| — | Milestone releases (ADR-012 addendum): roadmap milestones `v0.4`–`v0.6` + backlog, `milestone-release.yml` (CI dispatch for bot PRs, Release-As retargeting, release PR prepared for a maintainer merge by default, opt-in squash auto-merge via `AUTO_RELEASE_MERGE=true`, milestone close), PR milestone assignment from labels/closing references, trusted-actor event gate, `AUTO_RELEASE` / `release:hold` pause | 🚧 in review | `logic.test.ts` unit tests; end-to-end run only after merge |
