...
| 22 | AI Hub (`/hub`, ARCHITECTURE §4.16): catalog of 37 AI tools with categories and sub-categories, search/sort/filter in the URL, connection state from real provider accounts, registry models and usage ledger (live) or simulated in the browser (demo), connect wizard storing keys through the admin provider endpoint, vendored Simple Icons logos with sanitiser | 🚧 in review | 33 vitest tests (filter, URL state, catalog honesty, services, SVG sanitiser) · web typecheck + build · screenshot QA dark/light/375px |
